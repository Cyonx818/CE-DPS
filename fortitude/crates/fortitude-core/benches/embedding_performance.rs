...
use fortitude_core::vector::{EmbeddingService, MockEmbeddingService, VectorConfig};
...
fn benchmark_batch_embedding_generation(c: &mut Criterion) {
    ...
    for &batch_size in &batch_sizes {
        let texts: Vec<String> = (0..batch_size).map(|i| format!("test_text_{i}")).collect();

        group.throughput(Throughput::Elements(batch_size as u64));
        group.bench_with_input(BenchmarkId::new("batch", batch_size), &texts, |b, texts| {
            b.to_async(&rt).iter(|| async {
                let text_refs: Vec<&str> = texts.iter().map(|s| s.as_str()).collect();
                let _embeddings = service
                    .generate_batch_embeddings(black_box(text_refs))
                    .await;
